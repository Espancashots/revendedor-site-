...
const PRICE_API=`${SUPABASE_URL}/functions/v1/reseller-price-settings-api`
...
async function priceApi(body){const s=await session();if(!s)throw Object.assign(new Error('Sessão expirada.'),{code:'unauthorized'});const r=await fetch(PRICE_API,{method:'POST',headers:{'Content-Type':'application/json','apikey':SUPABASE_KEY,'Authorization':`Bearer ${s.access_token}`},body:JSON.stringify(body)});const d=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(d.detail||d.error||`Erro ${r.status}`);e.code=d.error;e.data=d;throw e}return d}
...
function applyPermissions(){
  ...
  const order=['overview','keys','clients','revenue','values'];if(permissions[activePanel]===false){activePanel=order.find(x=>permissions[x]!==false)||'overview'}
}
...
function setPanel(name){if(permissions[name]===false)return;activePanel=name;document.querySelectorAll('.panel').forEach(p=>p.classList.add('hidden'));$(`${name}Panel`).classList.remove('hidden');document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.panel===name));$('pageTitle').textContent={overview:'Overview',keys:'Keys',clients:'Clientes',revenue:'Receita',values:'Valores das keys'}[name]||name;$('sideNav').closest('.sidebar')?.classList.remove('open');loadPanel(name).catch(handleError)}
async function loadPanel(name){$('syncStatus').textContent='Atualizando…';if(name==='overview')await loadOverview();if(name==='keys')await loadLicenses();if(name==='clients')await loadClients();if(name==='revenue')await loadRevenue();if(name==='values')await loadPriceSettings();$('syncStatus').textContent='Atualizado'}
...
async function bootApp(){
  try{me=await api({action:'me'});permissions=me.permissions||{};await loadPriceSettings();setIdentity();applyPermissions();hide('authView');hide('blockedView');show('appView');setPanel(activePanel)}catch(e){handleError(e)}
}
...
function refreshPlanLabels(){
  for(const id of ['createPlan','renewPlan']){
    const select=$(id);if(!select)continue
    for(const option of select.options){const p=PLAN[option.value];if(p)option.textContent=`${p.label} — ${money(p.price)}`}
  }
}
function fillPriceSettings(){
  const map={'3h':'price3h','10h':'price10h','1d':'price1d','3d':'price3d','1w':'price1w','1m':'price1m'}
  for(const [code,id] of Object.entries(map)){if($(id))$(id).value=PLAN[code].price}
  refreshPlanLabels()
}
async function loadPriceSettings(){
  const d=await priceApi({action:'get'})
  for(const code of Object.keys(PLAN)){const n=Number(d.prices?.[code]);if(Number.isFinite(n)&&n>=0)PLAN[code].price=n}
  fillPriceSettings()
  return d
}
...
$('priceSettingsForm').addEventListener('submit',async e=>{
  e.preventDefault()
  const btn=$('savePricesButton');btn.disabled=true;btn.textContent='Salvando…'
  try{
    const prices={
      '3h':Number($('price3h').value),'10h':Number($('price10h').value),'1d':Number($('price1d').value),
      '3d':Number($('price3d').value),'1w':Number($('price1w').value),'1m':Number($('price1m').value)
    }
    if(Object.values(prices).some(v=>!Number.isFinite(v)||v<0)){flash('Confira os valores informados.');return}
    const d=await priceApi({action:'set',prices})
    for(const code of Object.keys(PLAN)){const n=Number(d.prices?.[code]);if(Number.isFinite(n)&&n>=0)PLAN[code].price=n}
    fillPriceSettings();flash('Valores das keys salvos.')
  }catch(err){handleError(err)}finally{btn.disabled=false;btn.textContent='Salvar valores'}
})
